...
class NeuralNetwork {
  ...
  */
  ...
  predict(input_array) {
    ...
    let hidden = Matrix.multiply(this.weights_ih, inputs);    // Generating the Hidden Outputs
    hidden.add(this.bias_h);
    hidden.map(this.activation_function.func);    // activation function!

    let outputs = Matrix.multiply(this.weights_ho, hidden);    // Generating the output's output!
  ...
  train(input_array, target_array) {
    ...
    let output_errors = Matrix.subtract(targets, outputs);    // Calculate the ERROR = TARGETS - OUTPUTS

    let gradients = Matrix.map(outputs, this.activation_function.dfunc);    // Calculate gradient = outputs * (1 - outputs);
    gradients.multiply(output_errors);
    gradients.multiply(this.learning_rate);

    let hidden_T = Matrix.transpose(hidden);    // Calculate deltas
    ...
    let hidden_gradient = Matrix.map(hidden, this.activation_function.dfunc);    // Calculate hidden gradient
