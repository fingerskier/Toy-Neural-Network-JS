...
class Actor {
  static arctan = new ActivationFunction(
    x => 1 / Math.tan(x),
    y => 1 / ((y*y) + 1)
  )

  static binary = new ActivationFunction(
    x => (x>0) ? 1 : 0,
    y => (y!==0) ? 0 : Infinity
  )

  static elu = new ActivationFunction(
    x => (x>0) ? 2 * (Math.exp(x) - 1) : x,
    y => (y>0) ? y + 2 : 1
  )

  static identity = new ActivationFunction(
    x => x,
    y => 1
  )
  ...
  static sigmoid = new ActivationFunction(
    x => 1 / (1 + Math.exp(-x)),
    y => y * (1 - y)
  )  
    
  static tanh = new ActivationFunction(
    x => Math.tanh(x),
    y => 1 - (y * y)
  )  

  static softplus = new ActivationFunction(
...
class NeuralNetwork {
  ...
  */
  constructor(in_nodes, hid_nodes, out_nodes) {
    ...
    this.activation_function = Actor.sigmoid
  }
